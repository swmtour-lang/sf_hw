// tb_rst_injection.v - Testbench for RST injection attack detection
`timescale 1ns / 1ps

module tb_rst_injection;
    ...
    // Task to send a SYN packet (establish connection)
    ...
    // Task to send a SYN-ACK packet (server response)
    task send_syn_ack_packet;
        input [31:0] src_ip;
        input [31:0] dst_ip;
        input [15:0] src_port;
        input [15:0] dst_port;
        ...
        begin
            ...
            // Similar to SYN packet but with SYN+ACK flags
            ...
            packet[14] = 8'h45;
            packet[15] = 8'h00;
            packet[16] = 8'h00; packet[17] = 8'h3C;
            ...
            packet[38] = seq_num[31:24]; packet[39] = seq_num[23:16]; packet[40] = seq_num[15:8]; packet[41] = seq_num[7:0];
            packet[42] = ack_num[31:24]; packet[43] = ack_num[23:16]; packet[44] = ack_num[15:8]; packet[45] = ack_num[7:0];
            packet[46] = 8'h50;
            packet[47] = 8'h12; // Flags (SYN + ACK)
    ...
    // Task to send an ACK packet (complete handshake)
    task send_ack_packet;
        ...
        begin
            ...
            packet[42] = ack_num[31:24]; packet[43] = ack_num[23:16]; packet[44] = ack_num[15:8]; packet[45] = ack_num[7:0];
            packet[46] = 8'h50;
            packet[47] = 8'h10; // Flags (ACK only)
            packet[48] = 8'h20; packet[49] = 8'h00;
            packet[50] = 8'h00; packet[51] = 8'h00;
            packet[52] = 8'h00; packet[53] = 8'h00;
    ...
    // Task to send a RST packet
    task send_rst_packet;
        input [31:0] src_ip;
        input [31:0] dst_ip;
        input [15:0] src_port;
        input [15:0] dst_port;
        ...
        begin
            ...
            packet[36] = dst_port[15:8]; packet[37] = dst_port[7:0];
            packet[38] = seq_num[31:24]; packet[39] = seq_num[23:16]; packet[40] = seq_num[15:8]; packet[41] = seq_num[7:0];
            packet[42] = 8'h00; packet[43] = 8'h00; packet[44] = 8'h00; packet[45] = 8'h00;
            packet[46] = 8'h50;
            packet[47] = 8'h04; // Flags (RST)
            packet[48] = 8'h00; packet[49] = 8'h00;
            packet[50] = 8'h00; packet[51] = 8'h00;
            packet[52] = 8'h00; packet[53] = 8'h00;
    ...
    initial begin
        ...
        $display("Starting RST injection test...");

        // Establish a legitimate connection
        $display("Establishing legitimate TCP connection...");
        send_syn_packet(32'hC0A80164, 32'hC0A80101, 16'd12345, 16'd80, 32'h00000001); // Client SYN
        #200;
        send_syn_ack_packet(32'hC0A80101, 32'hC0A80164, 16'd80, 16'd12345, 32'h00000002, 32'h00000002); // Server SYN-ACK
        #200;
        send_ack_packet(32'hC0A80164, 32'hC0A80101, 16'd12345, 16'd80, 32'h00000002, 32'h00000003); // Client ACK
        #200;

        // Send legitimate RST to close connection
        $display("Sending legitimate RST to close connection...");
        send_rst_packet(32'hC0A80164, 32'hC0A80101, 16'd12345, 16'd80, 32'h00000002); // Legitimate RST
        #200;

        // Send RST injection attack - wrong sequence number
        $display("Sending RST injection attack (wrong sequence number)...");
        send_rst_packet(32'hC0A80165, 32'hC0A80101, 16'd12346, 16'd80, 32'h000000FF); // Attacker RST with wrong seq
        #200;

        // Send multiple RST packets from same IP (rate limiting)
        $display("Sending multiple RST packets (rate limiting test)...");
        repeat (5) begin
            send_rst_packet(32'hC0A80166, 32'hC0A80101, 16'd12347, 16'd80, 32'h00000002);
            #100;
        end
    ...
    // Monitor results
    always @(posedge rst_injection_alert) begin
        $display("RST INJECTION DETECTED at time %t!", $time);
    end

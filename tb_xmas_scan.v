// tb_xmas_scan.v - Testbench for Xmas scan detection
`timescale 1ns / 1ps
...
module tb_xmas_scan;
    ...
    // Test task to send a Xmas scan packet (FIN+PSH+URG)
    task send_xmas_packet;
        input [31:0] src_ip;
        input [31:0] dst_ip;
        input [15:0] src_port;
        input [15:0] dst_port;
        begin
            ...
            packet[47] = 8'h29; // Flags (FIN=0x01, PSH=0x08, URG=0x20) = 0x29
    ...
    // Test task to send a normal TCP packet
    task send_normal_tcp_packet;
        input [31:0] src_ip;
        input [31:0] dst_ip;
        input [15:0] src_port;
        input [15:0] dst_port;
    ...
    initial begin
        ...
        $display("Starting Xmas scan test...");

        // Send some normal packets first
        send_normal_tcp_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd80); // 192.168.1.1 -> 192.168.1.2:12345->80
        #100;

        // Send Xmas scan packets from same IP
        send_xmas_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd80); // First Xmas packet
        #100;
        send_xmas_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd81); // Second Xmas packet
        #100;
        send_xmas_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd82); // Third Xmas packet
        #100;
        send_xmas_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd83); // Fourth Xmas packet
        #100;
        send_xmas_packet(32'hC0A80101, 32'hC0A80102, 16'd12345, 16'd84); // Fifth Xmas packet - should trigger alert
        #100;

        // Check if alert was triggered
        if (xmas_scan_alert) begin
            $display("SUCCESS: Xmas scan alert triggered after 5 packets");
        end else begin
            $display("FAILURE: Xmas scan alert not triggered");
        end
